...
class Tello:
    def __init__(self, tello_ip: str='192.168.10.1', debug: bool=True):
        ...
        self.heart_beat_freq = 10

        # For queueing commands
        self.send_commandQueue = deque()
        self.send_commandThread = threading.Thread(target=self.send_command2)
        self.send_commandThread.start()
        

        # Setting Tello to command mode
        self.command()

    # send_command will populate a deque with commands
    def send_command(self, command: str, query: bool = False):
        print("Command received: ", command)
        #Populate list with given command
        # self.log.append(Stats(command, len(self.log)))
        self.send_commandQueue.append((command, query))

    def send_command2(self):
        lastExecutedCommand = time.time()
        while(True):
            # print("Number of commands pending: ", len(self.send_commandQueue))
            if len(self.send_commandQueue) > 0:
                print("Commands in queue: ", self.send_commandQueue)
                command, query = self.send_commandQueue.popleft()

                #Execute command
                self.execute(command, query)
                lastExecutedCommand = time.time()
            elif (time.time() - lastExecutedCommand) > self.heart_beat_freq:
                #Send a heart beat command
                self.get_battery() 

    def execute(self, command: str, query: bool = False):
